...
export default async function handler(req,res){
 ...
 if(!configured())return res.status(503).json({error:'Online proposals are being connected. Please email make@creatorfox.com.'});
 ...
 try{
  ...
  const owner=process.env.LEAD_NOTIFICATION_EMAIL||'make@creatorfox.com';
 ...
 }catch(e){console.error('Enquiry processing failed:',e.name);return res.status(503).json({error:'We couldn’t confirm delivery. Please try again later or email make@creatorfox.com.'})}
}
